...
class Player(GameSprite):
    ...
    def fire(self):
        bullet = Bullet('bullet.png', self.rect.centerx - 5, self.rect.top, 10, 20, 20)
...
class Enemy(GameSprite):
    ...
    def update(self):
        ...
        if not self.bullet:
            if self.rect.y >= n:
                self.bullet.add(bullet('asteroid.png', self.rect.centerx - 5, self.rect.bottom, 20, 20, 5))
...
score_font = FontClass('Счет: ', 5, 10)
missed_font = FontClass('Пропущено: ',  5, 30)

enemys = sprite.Group()
for i in range(5):
    enemys.add(
        Enemy('ufo.png', randint(0, 635), -70, 65, 65, randint(1, 3))
    )
hero = Player('rocket.png', 317.5, 420, 65, 65, 5)
...
while game:
    ...
    if not finish:
        ...
        if groupcollides:
            ...
            boom = GameSprite('boom.png', collide.rect.x, collide.rect.y, 65, 65, 0)
            boom.reset()
        window.blit(background, (0, 0))
        hero.reset()
        hero.update()
        ...
        if spritecollide or lost > 3:
            finish = True
            boom = GameSprite('boom.png', hero.rect.centerx - 82.5, hero.rect.top - 82.5, 165, 165, 0)
            boom.reset()
            mixer.music.stop()
            lose.reset()
        elif score >= 9:
